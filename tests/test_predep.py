...
class TestPredep:
    ...
    def test_predep_constant_data(self):
        ...
        # Com dados constantes, pode retornar NaN
        assert np.isnan(result) or isinstance(result, (float, np.floating))
    ...
    def test_predep_different_n_boot(self):
        ...
        assert isinstance(result_1k, (float, np.floating)) or np.isnan(result_1k)
        assert isinstance(result_10k, (float, np.floating)) or np.isnan(result_10k)